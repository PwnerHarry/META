...
# TODO: this file hasn't been changed to TD yet, it is just GTD!
...
def true_online_td(env, episodes, target, behavior, Lambda, gamma = lambda x: 0.95, alpha = 0.05, beta = 0.0001, diagnose = False, evaluate = None):
    """
...
def eval_togtd_per_run(env, truth, stat_dist, runtime, runtimes, episodes, target, behavior, gamma, Lambda, alpha, beta, evaluate):
    ...
    value_trace = true_online_td(env, episodes, target, behavior, Lambda, gamma = gamma, alpha = alpha, beta = beta, evaluate = evaluate)
    if evaluate is not None:
