...
def MTA(env, steps, target, behavior, evaluate, Lambda, encoder, learner_type='togtd', gamma=lambda x: 0.95, alpha=0.05, beta=0.05, kappa=0.01, parametric_lambda=True):
    ...
    while step < steps:
        ...
        try:
            while not done:
                ...
                if step > 0.1 * steps and L_var_next > np.sqrt(np.finfo(float).eps):
            ...
            print('RuntimeWarning captured, possibly due to numerical stability issues'); break
    warnings.filterwarnings("default")
    return value_trace

def eval_MTA_per_run(env_name, runtime, runtimes, steps, target, behavior, kappa, gamma, Lambda, alpha, beta, evaluate, encoder, learner_type, parametric_lambda):
    ...
    value_trace = MTA(env, steps, target, behavior, evaluate, Lambda, encoder, learner_type='togtd', gamma=gamma, alpha=alpha, beta=beta, kappa=kappa, parametric_lambda=parametric_lambda)
    return value_trace.reshape(1, -1)

def eval_MTA(env_name, behavior, target, kappa, gamma, alpha, beta, runtimes, steps, evaluate, encoder, learner_type='togtd', parametric_lambda=True):
    env = gym.make(env_name)
    if parametric_lambda:
    ...
    results = Parallel(n_jobs=-1)(delayed(eval_MTA_per_run)(env_name, runtime, runtimes, steps, target, behavior, kappa, gamma, LAMBDAS[runtime], alpha, beta, evaluate, encoder, learner_type, parametric_lambda) for runtime in range(runtimes))
